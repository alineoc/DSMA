...
setwd("C:/Users/aline/Documents/Goethe Uni Master/DSMA/Term Paper Dateien aus OLAT") #set working directory
...
# own analysis Aline
print(yelp_data)
summary_ana = summary (yelp_data)
print(summary_ana)
describe(summary_ana)
describeBy(yelp_data)
write.xlsx(summary_ana, "table_summary_ana.xlsx")

# correlation matrix
library("Hmisc")

choice=c("ch_in","WE","Quarter","business_price","business_park","TOBS","PRCP","n_photo","female","male","cum_n_review","cum_max_friends","cum_max_u_elite","cum_max_us_fans","cum_max_us_rev")
my_data <- subset(yelp_data,select=choice)
print(my_data)
...
library(xlsx)
describe(my_data)
summary = describeBy(my_data)
describeBy(my_data)
write.xlsx(summary, "table_summary.xlsx")

my_data$WE <- ifelse(my_data$WE == 'TRUE', 1, 0)

my_data$Summer <- ifelse(my_data$Quarter == 'Q3', 1, 0)

my_data$business_park <- ifelse(my_data$business_park == 'true', 1, 0)
print(my_data)

my_data <- subset(my_data, select = -c(Quarter))
print(my_data)
...
yelp_data_analysis= yelp_data %>%
#  group_by(business_id) %>%
  summarize(ch_ins=sum(ch_in,na.rm=T),business_price=mean(business_price,na.rm=T),
            n_photo=mean(n_photo,na.rm=T),cum_n_review=mean(cum_n_review,na.rm=T),cum_max_friends=mean(cum_max_friends,na.rm=T),
...
varsin=c("ch_in_string","ch_in","WE","Quarter","business_price","business_cat","business_park","TOBS","PRCP","n_photo","female","male","cum_n_review","cum_max_friends","cum_max_u_elite","cum_max_us_fans","cum_max_us_rev")
...
x.evaluate$predictionNBclass[x.evaluate$predictionNB[,'Noch_in']>probthres]="Noch_in"
x.evaluate$predictionNBclass[x.evaluate$predictionNB[,'Noch_in']<=probthres]="ch_in"
...
x.evaluate$predictionKNNclass[x.evaluate$predictionKNN[,'Noch_in']>probthres]="Noch_in"
x.evaluate$predictionKNNclass[x.evaluate$predictionKNN[,'Noch_in']<=probthres]="ch_in"
...
x.evaluate$predictionSVMclass[x.evaluate$predictionSVM[,'Noch_in']>probthres]="Noch_in"
x.evaluate$predictionSVMclass[x.evaluate$predictionSVM[,'Noch_in']<=probthres]="ch_in"
...
# Extract the class probabilities.
x.evaluate$predictionSVM <- x.evaluate$predictionSVM[,'Noch_in']
...
x.evaluate$predictionNNetclass[x.evaluate$predictionNNet[,"Noch_in"]>probthres]="Noch_in"
x.evaluate$predictionNNetclass[x.evaluate$predictionNNet[,"Noch_in"]<=probthres]="ch_in"
...
x.evaluate$predictionTreeClass[x.evaluate$predictionTree[,"Noch_in"]>probthres]="Noch_in"
x.evaluate$predictionTreeClass[x.evaluate$predictionTree[,"Noch_in"]<=probthres]="ch_in"
...
x.evaluate$predictionBaggingClass[x.evaluate$predictionBagging[,"Noch_in"]>probthres]="Noch_in"
x.evaluate$predictionBaggingClass[x.evaluate$predictionBagging[,"Noch_in"]<=probthres]="ch_in"
...
x.evaluate$predictionBoostingClass[x.evaluate$predictionBoosting[,"Noch_in"]>probthres]="Noch_in"
x.evaluate$predictionBoostingClass[x.evaluate$predictionBoosting[,"Noch_in"]<=probthres]="ch_in"
...
x.evaluate$predictionRFClass[x.evaluate$predictionRF[,"Noch_in"]>probthres]="Noch_in"
x.evaluate$predictionRFClass[x.evaluate$predictionRF[,"Noch_in"]<=probthres]="ch_in"
...
#barplot(c(LogitOutput$GINI,naiveBayesOutput$GINI, SVMOutput$GINI,TreeOutput$GINI,BaggingOutput$GINI,BoostingOutput$GINI,RFOutput$GINI,NNetOutput$GINI), names.arg = c("Logit","Naive Bayes","SVM","Tree","Bagging","Boosting","Random F.","Neural Network"), main="GINI coefficients of the models")

OverallTDL <- c(LogitOutput$TDL,SVMOutput$TDL,TreeOutput$TDL,BaggingOutput$TDL,BoostingOutput$TDL,RFOutput$TDL,KNNOutput$TDL,NBOutput$TDL,NNetOutput$TDL)
OverallGINI <- c(LogitOutput$GINI,SVMOutput$GINI,TreeOutput$GINI,BaggingOutput$GINI,BoostingOutput$GINI,RFOutput$GINI,KNNOutput$GINI,NBOutput$GINI,NNetOutput$GINI)
...
barplot(t(as.matrix(ForGraph[, c("OverallTDL1", "OverallGINI1")])), beside = TRUE, yaxt = "n", names.arg = c("Logit","SVM","Tree","Bagging","Boosting","RF","KNN","NB","ANN"), ylim=c(0, max(c(myLeftAxisAt, myRightAxisAt))), ylab =	"Top Decile Lift", legend = c("TDL","GINI"), main="Performance of the Machine Learning Algorithms")
...
mtext(c(paste(round(LogitOutput$TimeElapsed,digits=2),"sec"),
        ...
        paste(round(NNetOutput$TimeElapsed,digits=2),"sec")), side = 1, line = 3, cex = par("cex.lab"), at = c(2,6,10,14,18,22,26))
mtext(c(paste(round(LogitOutput$PercCorrect,digits=0),"%"),
        ...
        paste(round(NNetOutput$PercCorrect,digits=0),"%")), side = 1, line = 4, cex = par("cex.lab"), at = c(5,8,12,15,18,21,24))

mtext("Calc. time", side = 1, line = 3, cex = par("cex.lab"), at = -.8)
mtext("% correct", side = 1, line = 4, cex = par("cex.lab"), at = -.8)

# print accuracy and run time seperate since overlappings in plot
print(LogitOutput$TimeElapsed)
print(SVMOutput$TimeElapsed)
print(TreeOutput$TimeElapsed)
print(BaggingOutput$TimeElapsed)
print(BoostingOutput$TimeElapsed)
print(RFOutput$TimeElapsed)
print(KNNOutput$TimeElapsed)
print(NBOutput$TimeElapsed)
print(NNetOutput$TimeElapsed)

print(LogitOutput$PercCorrect)
print(SVMOutput$PercCorrect)
print(TreeOutput$PercCorrect)
print(BaggingOutput$PercCorrect)
print(BoostingOutput$PercCorrect)
print(RFOutput$PercCorrect)
print(KNNOutput$PercCorrect)
print(NBOutput$PercCorrect)
print(NNetOutput$PercCorrect)
